...
for task in capture preprocess infer fuse report; do
  docker build \
    ...
    -t $REGISTRY/dsf-iobt-$task:latest . \
  && docker push $REGISTRY/dsf-iobt-$task:latest
done
```
...
```bash
dsf odag submit --template iobt-mission-snapshot
```

### 5. Monitor

```bash
dsf odag status iobt-mission-snapshot-run-1
dsf odag logs iobt-mission-snapshot-run-1 capture-1
dsf odag logs iobt-mission-snapshot-run-1 generate-report
```
...
### capture (capture-1..4)
Generates a deterministic raw blob (MD5-seeded repeating blocks) of `DSF_DATA_SIZE` bytes.
Simulates sensor capture delay via `sleep(DSF_RUNTIME)`. Sends raw bytes to preprocess.
...
### infer (infer-1..4)
Receives feature bytes, runs a CPU busy loop for `DSF_RUNTIME` seconds (SHA-256 hashing
