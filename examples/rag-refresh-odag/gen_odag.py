...
def generate(nodes, registry, template_name):
    ...
    ingest = task_block("ingest-shard", f"{registry}/dsf-rag-ingest:latest",
    ...
    indices = ""
    for i in range(1, 5):
        chunks += task_block(f"chunk-shard-{i}", f"{registry}/dsf-rag-chunk:latest",
                             ["ingest-shard"], 6, 5, "300m", "256Mi", [data_node])
        embeds += task_block(f"embed-shard-{i}", f"{registry}/dsf-rag-embed:latest",
                             [f"chunk-shard-{i}"], 5, 20, "500m", "256Mi", embed_nodes)
        indices += task_block(f"build-index-{i}", f"{registry}/dsf-rag-index:latest",
                              [f"embed-shard-{i}"], 6, 3, "300m", "256Mi")

    merge = task_block("merge-index", f"{registry}/dsf-rag-merge:latest",
                       [f"build-index-{i}" for i in range(1, 5)],
                       22, 5, "300m", "256Mi", merge_nodes)

    ev = task_block("eval-queries", f"{registry}/dsf-rag-eval:latest",
                    ["merge-index"], 1, 15, "500m", "256Mi", merge_nodes)

    rpt = task_block("report", f"{registry}/dsf-rag-report:latest",
