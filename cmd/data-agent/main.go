...
//   - Task state: <rel>/.dsf-task-state ∈ {Pending, Running, ComputeDone, Failed}
//   - Local data availability: presence of <rel>/.dsf-ready marker
...
//                                      then writes .dsf-ready on success
//   GET /<odag>/<task>/output        — read output (requires .dsf-ready unless ?unsafe=1)
//
...
//
//   taskStateFile  →  .dsf-task-state  (text: one of {Pending, Running,
//                                       ComputeDone, Failed}; task lifecycle only,
//                                       independent of data availability)
//   readyFile      →  .dsf-ready       (presence-only marker: the producer's
//                                       output is atomically installed on this
//                                       node and safe for a consumer to read)
//   sendingFile    →  .dsf-sending     (true/false: data-agent is mid-transfer)
//   bytesFile      →  .dsf-bytes       (size of the installed output)
func taskStateFile(rel string) string  { return filepath.Join(dataDir, filepath.Clean(rel), ".dsf-task-state") }
func readyFile(rel string) string      { return filepath.Join(dataDir, filepath.Clean(rel), ".dsf-ready") }
...
func digestSidecarFile(rel string) string { return filepath.Join(dataDir, filepath.Clean(rel), ".dsf-sha256") }
...
// <rel>/.dsf-task-state. Invalid values are rejected with a warning so callers
...
// setReady creates the .dsf-ready marker on this node, durably. The marker
...
// clearReady removes the .dsf-ready marker. Used on controller-driven reset.
...
// isReady reports whether <rel>/.dsf-ready exists.
...
func main() {
	...
	// Presence of <rel>/.dsf-ready means the producer's output is atomically
	...
	// This enforces the invariant "the only way to create .dsf-ready is
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPut:
			...
			// (.dsf-ready present) with the same digest, return 200 without
			...
			// never touched (temp file is removed). The .dsf-ready marker is
			...
			w.WriteHeader(http.StatusOK)
		case http.MethodGet:
			// Default: require .dsf-ready. Files may exist on disk before
