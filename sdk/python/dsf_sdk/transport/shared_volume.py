"""
Shared-volume transport for DSF — centralized storage baseline.

All tasks read/write through a shared NFS volume instead of DSF's native
...
Env vars:
    DSF_SHARED_DIR    — shared mount path (default: /shared/dsf-outputs)
    DSF_ODAG_NAME     — ODAG name (used as directory prefix)
...
class SharedVolumeTransport:
    ...
    Sender writes to /shared/dsf-outputs/<odag>/<task>/output.
    ...
    def __init__(self) -> None:
        self._shared_dir = os.environ.get("DSF_SHARED_DIR", "/shared/dsf-outputs")
        self._odag_name = os.environ.get("DSF_ODAG_NAME", "unknown")
        self._task_name = os.environ.get("DSF_TASK_NAME", "unknown")
        self._deps = [d for d in os.environ.get("DSF_DEPS", "").split(",") if d]
        self._succs = [s for s in os.environ.get("DSF_SUCCESSORS", "").split(",") if s]
