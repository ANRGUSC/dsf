...
for task in ingest chunk embed index merge eval report; do
  docker build \
    ...
    -t $REGISTRY/dsf-rag-$task:latest . \
  && docker push $REGISTRY/dsf-rag-$task:latest
done
```
...
```bash
dsf odag run rag-refresh -n dsf-system
```

### 5. Monitor

```bash
dsf odag status rag-refresh-run-001 -n dsf-system
dsf odag logs rag-refresh-run-001 merge-index -n dsf-system
dsf odag logs rag-refresh-run-001 report -n dsf-system
```
...
```bash
dsf odag run rag-refresh -n dsf-system   # creates run-002, run-003, ...
```
