...
```bash
cd /home/anrg/dsf/phase-1/Schedulers/src/heft-scheduler
docker buildx build -t <your-registry>/heft-scheduler:latest --push .
```
...
kubectl logs deployment/heft-scheduler -n kube-system | grep "\[HEFT\]"

# View schedule decisions
kubectl logs deployment/heft-scheduler -n kube-system | grep "Task.*-> Node"
```
...
**Solution**: Check if DAG was detected. Ensure CRD has `schedulerName: heft-scheduler`

### Pods not being scheduled
**Issue**: Pods stuck in Pending
**Solution**: 
- Check scheduler logs: `kubectl logs deployment/heft-scheduler -n kube-system`
