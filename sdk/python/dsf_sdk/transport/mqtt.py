"""
MQTT transport implementation for DSF — centralized broker baseline.

All messages route through a single MQTT broker (Mosquitto). This serves
as a comparison point against DSF's native P2P ZeroMQ transport to
demonstrate the scalability advantage of direct communication.

Topic convention:
    dsf/<cdag-name>/<src-task>/broadcast   — broadcast messages
    dsf/<cdag-name>/<src-task>/to/<target>  — targeted messages

Env vars:
    DSF_MQTT_BROKER   — broker address (default: mqtt-broker.dsf-system.svc.cluster.local)
    DSF_MQTT_PORT     — broker port (default: 1883)
    DSF_ODAG_NAME or DSF_CDAG_NAME — used as topic prefix
"""
...
class MqttTransport:
    """
    MQTT-based transport for DSF evaluation.

    All messages go through a centralized MQTT broker — this is the
    baseline against which we compare DSF's P2P transport.
    """
    ...
    def __init__(self, peer_endpoints: dict[str, str]) -> None:
        ...
        self._dag_name = os.environ.get("DSF_CDAG_NAME", os.environ.get("DSF_ODAG_NAME", "unknown"))
        self._broker = os.environ.get("DSF_MQTT_BROKER", "mqtt-broker.dsf-system.svc.cluster.local")
        self._port = int(os.environ.get("DSF_MQTT_PORT", "1883"))
        ...
        # Connect to broker.
        self._client = mqtt.Client(client_id=f"dsf-{self._dag_name}-{self._task_name}", protocol=mqtt.MQTTv5)
        self._client.on_message = self._on_message
        self._client.connect(self._broker, self._port, keepalive=60)
        self._client.loop_start()

        # Subscribe to broadcast and targeted topics for this task.
        self._client.subscribe(f"dsf/{self._dag_name}/+/{self.BROADCAST_SUFFIX}", qos=0)
        self._client.subscribe(f"dsf/{self._dag_name}/+/to/{self._task_name}", qos=0)

        time.sleep(0.3)  # let subscriptions propagate

    def _on_message(self, client, userdata, msg):
        """Callback: route incoming message to the right inbox."""
        # Extract source task from topic: dsf/<dag>/<src>/broadcast or dsf/<dag>/<src>/to/<target>
    ...
    def send(self, payload: bytes) -> None:
        """Publish broadcast."""
        topic = f"dsf/{self._dag_name}/{self._task_name}/{self.BROADCAST_SUFFIX}"
    ...
    def publish(self, payload: bytes, topic: bytes = b"") -> None:
        ...
        if target and target != "*":
            mqtt_topic = f"dsf/{self._dag_name}/{self._task_name}/to/{target}"
        else:
            mqtt_topic = f"dsf/{self._dag_name}/{self._task_name}/{self.BROADCAST_SUFFIX}"
