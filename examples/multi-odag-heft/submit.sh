...
# under contention. Uses kubectl (or: dsf odag submit -f <file>).
#
# Usage:
...
cleanup() {
  ...
  for name in "${NAMES[@]}"; do
    if kubectl get odag -n dsf-system "$name" &>/dev/null; then
      echo "  deleting odag/$name ..."
      kubectl delete odag -n dsf-system "$name" --ignore-not-found
    fi
  done
...
if ! $DRY_RUN; then
  echo "=== Monitoring ==="
  echo "  kubectl get odags -n dsf-system -w"
  echo "  kubectl get pods -n dsf-system -l app=dsf-odag -w"
  echo ""
  echo "=== Quick status loop (Ctrl-C to stop) ==="
  while true; do
    echo "--- $(date +%H:%M:%S) ---"
    kubectl get odags -n dsf-system -o custom-columns=\
NAME:.metadata.name,PHASE:.status.phase,SCHEDULER:.spec.scheduler,MAKESPAN:.status.makespan,AGE:.metadata.creationTimestamp \
    ...
    total=$(kubectl get odags -n dsf-system --no-headers 2>/dev/null | wc -l)
    succeeded=$(kubectl get odags -n dsf-system --no-headers 2>/dev/null | grep -c "Succeeded" || true)
    failed=$(kubectl get odags -n dsf-system --no-headers 2>/dev/null | grep -c "Failed" || true)
