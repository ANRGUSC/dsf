...
Two patterns are supported, selected by DSF_TRANSPORT_PATTERN env var:
  ...
  pubsub (for continuous CTGs):
    ...
    - Each message carries a send-timestamp frame so subscribers can
      ...
      DSF_METRICS_PORT (default 8090) exposes rolling counters.
...
_METRICS_PORT = int(os.environ.get("DSF_METRICS_PORT", "8090"))
...
class _MetricsRecorder:
    ...
    def snapshot(self) -> dict:
        with self._lock:
            ...
            return {
                "taskName": os.environ.get("DSF_TASK_NAME", ""),
                "odagOrCdagName": os.environ.get("DSF_CDAG_NAME", os.environ.get("DSF_ODAG_NAME", "")),
...
def _start_metrics_server_once() -> None:
    ...
    def _run() -> None:
        try:
            ...
            print(f"[dsf_sdk] metrics HTTP server exited: {e}", flush=True)

    t = threading.Thread(target=_run, daemon=True, name="dsf-metrics")
    t.start()
